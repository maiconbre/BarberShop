...
const Stats: React.FC<StatsProps> = ({ appointments, revenueDisplayMode, setRevenueDisplayMode }) => {
  ...
  const generateCacheKey = useCallback((mode: string, appointmentsLength: number) => {
    // Incluir hash dos status para invalidar cache quando status mudam
    const statusHash = appointments.map(app => `${app.id}:${app.status}`).join('|');
    const simpleHash = appointments.length > 0 ? appointments[0]?.id + appointments[appointments.length - 1]?.id : 'empty';
    return `${mode}_${appointmentsLength}_${simpleHash}_${statusHash.slice(0, 50)}`; // Limitar tamanho do hash
  }, [appointments]);
  ...
  const calculateStatsWithCache = useCallback((mode: string) => {
    ...
    if ((currentUser as { role?: string; id?: string })?.role === 'barber' && (currentUser as { id?: string })?.id) {

      userFilteredAppointments = appointments.filter(app => app.barberId === (currentUser as { id: string }).id);
    }
  ...
  }, [revenueDisplayMode, appointments, calculateStatsWithCache]);
  ...
  // Limpar cache quando appointments mudarem para forçar recálculo
  useEffect(() => {
    // Limpar apenas entradas antigas do cache para evitar acúmulo
    const now = Date.now();
  ...
  const getNextAppointment = useMemo(() => {
    ...
    const futureAppointments = userFilteredAppointments.filter(app => {
  ...
  }, [revenueDisplayMode, generateCacheKey, isCacheValid, setRevenueDisplayMode, appointments.length]);
        ...
        <div className="flex justify-center items-center gap-1 bg-[#252B3B]/30 p-0.5 rounded-lg">
          {(['month', 'week', 'day'] as const).map((mode) => (
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className={`relative px-2 py-1.5 text-xs rounded-md transition-all duration-300 flex-1 min-w-0 ${revenueDisplayMode === mode
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
              ...
              <div>
                ...
                {getNextAppointment?.appointment && (
                  <p className="text-xs text-gray-400 mt-1 truncate">
                    {getNextAppointment.appointment.time}
                  </p>
                )}
              </div>
              {getNextAppointment?.appointment && (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div className={`p-1.5 rounded-full ${clientesMes > clientesSemana / 4
                ? 'bg-green-500/10'
                : clientesMes < clientesSemana / 4
                  ? 'bg-red-500/10'
                  : 'bg-yellow-500/10'
                }`}>
                <TrendingUp className={`h-5 w-5 ${clientesMes > clientesSemana / 4
                  ? 'text-green-400'
                  : clientesMes < clientesSemana / 4
                    ? 'text-red-400'
                    : 'text-yellow-400'
