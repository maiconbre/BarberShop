...
const ClientAnalytics: React.FC<ClientAnalyticsProps> = ({ appointments }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-[#1A1F2E] to-[#252B3B] rounded-xl shadow-lg p-4 sm:p-6 mb-6 relative">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
          <div className="relative flex-1 sm:flex-none sm:w-64 min-w-[180px]">
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-[#0D121E] rounded-lg focus:ring-2 focus:ring-[#F0B35B] outline-none transition-all text-sm placeholder-gray-500"
            />
          ...
          <div className="flex gap-2">
            <button
              ...
              className="p-2 rounded-lg bg-[#0D121E] text-gray-400 hover:text-[#F0B35B] transition-colors relative"
            ...
            <button
              ...
              className="p-2 rounded-lg bg-[#0D121E] text-gray-400 hover:text-[#F0B35B] transition-colors sm:flex hidden"
          ...
          {filterModalOpen && (
            <div 
              ref={filterModalRef}
              className="absolute right-4 top-24 sm:top-20 z-10 bg-[#1A1F2E] rounded-lg shadow-xl border border-gray-700 p-4 w-[280px] sm:w-[320px]"
            >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select 
                    ...
                    className="w-full bg-[#0D121E] text-white text-sm rounded-md px-3 py-2 outline-none focus:ring-1 focus:ring-[#F0B35B]"
                  >
      ...
      {/* Filtros de período */}
      <div className="flex mb-6 gap-2 overflow-x-auto pb-2 hide-scrollbar">
        <button
          onClick={() => setTimeRange('week')}
          className={`px-3 py-1.5 text-xs rounded-md transition-all ${timeRange === 'week' ? 'bg-[#F0B35B] text-black' : 'bg-[#0D121E] text-white hover:bg-[#F0B35B]/20'}`}
        >
          Última Semana
        </button>
        <button
          onClick={() => setTimeRange('month')}
          className={`px-3 py-1.5 text-xs rounded-md transition-all ${timeRange === 'month' ? 'bg-[#F0B35B] text-black' : 'bg-[#0D121E] text-white hover:bg-[#F0B35B]/20'}`}
        >
          Último Mês
        </button>
        <button
          onClick={() => setTimeRange('quarter')}
          className={`px-3 py-1.5 text-xs rounded-md transition-all ${timeRange === 'quarter' ? 'bg-[#F0B35B] text-black' : 'bg-[#0D121E] text-white hover:bg-[#F0B35B]/20'}`}
        >
          Último Trimestre
        </button>
        <button
          onClick={() => setTimeRange('all')}
          className={`px-3 py-1.5 text-xs rounded-md transition-all ${timeRange === 'all' ? 'bg-[#F0B35B] text-black' : 'bg-[#0D121E] text-white hover:bg-[#F0B35B]/20'}`}
        >
          Todo Período
        </button>
      </div>
      ...
      {/* Tabs de navegação */}
      <div className="flex flex-wrap border-b border-gray-700 mb-6 overflow-x-auto hide-scrollbar">
        <button
          onClick={() => setActiveTab('clients')}
          className={`px-4 py-2 text-sm font-medium transition-colors whitespace-nowrap ${activeTab === 'clients' ? 'text-[#F0B35B] border-b-2 border-[#F0B35B]' : 'text-gray-400 hover:text-white'}`}
        >
          Clientes Frequentes
        </button>
        <button
          onClick={() => setActiveTab('services')}
          className={`px-4 py-2 text-sm font-medium transition-colors whitespace-nowrap ${activeTab === 'services' ? 'text-[#F0B35B] border-b-2 border-[#F0B35B]' : 'text-gray-400 hover:text-white'}`}
        >
          Serviços Populares
        </button>
        <button
          onClick={() => setActiveTab('recurrence')}
          className={`px-4 py-2 text-sm font-medium transition-colors whitespace-nowrap ${activeTab === 'recurrence' ? 'text-[#F0B35B] border-b-2 border-[#F0B35B]' : 'text-gray-400 hover:text-white'}`}
        >
          Recorrência
        </button>
        <button
          onClick={() => setActiveTab('trends')}
          className={`px-4 py-2 text-sm font-medium transition-colors whitespace-nowrap ${activeTab === 'trends' ? 'text-[#F0B35B] border-b-2 border-[#F0B35B]' : 'text-gray-400 hover:text-white'}`}
        >
          Tendências
        </button>
      </div>
      ...
      <div className="min-h-[300px]">
        ...
        {activeTab === 'services' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-[#0D121E] p-2 sm:p-3 md:p-4 rounded-lg">
              <h3 className="text-sm font-medium text-gray-300 mb-2 sm:mb-4 text-center">Serviços Mais Populares</h3>
              <div className="h-64">
            ...
            <div className="bg-[#0D121E] p-4 rounded-lg">
              <h3 className="text-sm font-medium text-gray-300 mb-4 text-center">Frequência por Dia da Semana</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                  ...
                  >
                    ...
                    <XAxis dataKey="name" tick={{ fontSize: window.innerWidth < 768 ? 8 : 10, fill: '#fff' }} />
                    <YAxis tick={{ fontSize: window.innerWidth < 768 ? 8 : 10, fill: '#fff' }} width={window.innerWidth < 768 ? 30 : 40} />
                    <Tooltip
                      contentStyle={{
                        backgroundColor: 'rgba(26,31,46,0.95)',
                        border: '1px solid rgba(240,179,91,0.5)',
        ...
        {activeTab === 'recurrence' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-[#0D121E] p-2 sm:p-3 md:p-4 rounded-lg">
              <h3 className="text-sm font-medium text-gray-300 mb-2 sm:mb-4 text-center">Recorrência de Clientes</h3>
              <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
            ...
            <div className="bg-[#0D121E] p-4 rounded-lg">
              <h3 className="text-sm font-medium text-gray-300 mb-4">Sugestões para Promoções</h3>
              <div className="space-y-4 text-sm">
                {filteredClients.length > 0 ? (
                  <>
                    <div className="p-3 bg-[#1A1F2E] rounded-lg border-l-2 border-[#F0B35B]">
                      <h4 className="font-medium text-white">Clientes Fiéis</h4>
                    ...
                    <div className="p-3 bg-[#1A1F2E] rounded-lg border-l-2 border-green-400">
                      <h4 className="font-medium text-white">Dia Menos Movimentado</h4>
                      <p className="text-gray-400 mt-1">
                    ...
                    <div className="p-3 bg-[#1A1F2E] rounded-lg border-l-2 border-blue-400">
                      <h4 className="font-medium text-white">Serviço para Promover</h4>
                      <p className="text-gray-400 mt-1">
                    ...
                    <div className="p-3 bg-[#1A1F2E] rounded-lg border-l-2 border-purple-400">
                      <h4 className="font-medium text-white">Recuperação de Clientes</h4>
                      <p className="text-gray-400 mt-1">
                  ...
                  <div className="flex flex-col items-center justify-center h-40 text-gray-400">
                    ...
                    <p>Sem dados suficientes para gerar sugestões</p>
                  </div>
                )}
              </div>
            </div>
        ...
        {activeTab === 'trends' && (
          <div className="grid grid-cols-1 gap-6">
            <div className="bg-[#0D121E] p-2 sm:p-3 md:p-4 rounded-lg">
              <h3 className="text-sm font-medium text-gray-300 mb-2 sm:mb-4 text-center">Tendências das Últimas 12 Semanas</h3>
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
                    data={trendsData}
                  ...
                  >
                    ...
                    <XAxis 
                      dataKey="name" 
                      tick={{ fontSize: window.innerWidth < 768 ? 8 : 10, fill: '#fff' }}
                    />
                    <YAxis 
                      yAxisId="left" 
                      orientation="left" 
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-6">
              <div className="bg-[#0D121E] p-2 sm:p-4 rounded-lg">
                ...
                <div className="flex flex-col items-center justify-center h-20 sm:h-24">
                  ...
                  <p className="text-[10px] sm:text-xs text-gray-400">por cliente</p>
                </div>
              </div>
              
              <div className="bg-[#0D121E] p-2 sm:p-4 rounded-lg">
                <h3 className="text-xs sm:text-sm font-medium text-gray-300 mb-1 sm:mb-2 text-center">Clientes Recorrentes</h3>
                <div className="flex flex-col items-center justify-center h-20 sm:h-24">
                  <Award className="h-6 w-6 text-[#F0B35B] mb-1" />
                  <p className="text-lg sm:text-2xl font-bold text-white">
                    {clientsData.filter(client => client.visits > 1).length}
                  </p>
                  <p className="text-[10px] sm:text-xs text-gray-400">retornaram</p>
                </div>
              </div>
              
              <div className="bg-[#0D121E] p-2 sm:p-4 rounded-lg">
                <h3 className="text-xs sm:text-sm font-medium text-gray-300 mb-1 sm:mb-2 text-center">Serviços por Cliente</h3>
                <div className="flex flex-col items-center justify-center h-20 sm:h-24">
                  <BarChart2 className="h-6 w-6 text-blue-400 mb-1" />
                  <p className="text-lg sm:text-2xl font-bold text-white">
                    {(clientsData.reduce((sum, client) => sum + Object.keys(client.services).length, 0) / clientsData.length || 0).toFixed(1)}
                  </p>
                  <p className="text-[10px] sm:text-xs text-gray-400">média</p>
                </div>
              </div>
            </div>
          </div>
